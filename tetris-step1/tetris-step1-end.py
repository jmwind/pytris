...
def find_in(x,y, array):
    result = [item for item in array if item[0] == x or item[1] == y]
    return len(result) > 0
...
def advanceBoxes(board):
    ...
    for x in range(BOARD_BLOCK_W):
        for y in reversed(range(BOARD_BLOCK_H)):
            if board[x][y] != BLANK and y+1 < BOARD_BLOCK_H and (board[x][y+1] == BLANK or find_in(x,y,moves)):
                moves.append((x,y))
    for (x,y) in moves:
